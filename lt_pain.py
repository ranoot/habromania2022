...
scaled_v = scaled_vector(dim["h"] - b_crop_h - crop_h, 0.9)
...
while True:
	...
	if curr:
		...
		green_lines = cv.HoughLines(gs_frame_edges,1,np.pi/180,40)
		curr = green_square(green_lines, green)
		...
		h, w = thresh.shape
		lines = cv.HoughLines(line_edges,1,np.pi/180,20)
		majik = 30
		...
		if lines is not None and black_ratio > 0.2:
			...
			for line in lines:
				...
				if within_hw(y_intercept, h) and blackish(thresh ,int(y_intercept), 0):
					# print("A")
					v[0] -= w/2
					v[1] += (h - y_intercept) 
				if within_hw(x_intercept, w) and blackish(thresh, 0, int(x_intercept)):
					...
					v[0] += (x_intercept - w/2)
					v[1] += h
				if within_hw(width_intercept, h) and blackish(thresh, int(width_intercept), w - 1):
					# print("C")
					v[0] += w/2
