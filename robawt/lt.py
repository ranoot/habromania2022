...
def d_vector(masked_img, scaled_v: np.ndarray) -> tuple[float, float]:
...
def green_square(lines, g_frame):
	"""
	g_frame is the green mask for the image
	"""
	global l
	if lines is None:
		return
	...
	if len(f_lines) > 0:
		...
		# Cropping and inRange isn't perfect and some green still remains on the screen
